...
def main():
    ...
    def FRM1():
        ...
        bt4.pack()
        bt6=Button(frm1,text='Cancel Ticket',height=3,width=25,command=lambda:FRMI())
        bt6.pack()
        ...
        def FRMP():
            ...
            def show(a):
                ...
                txt=txt+'\nPNR NO: '+h[0]
                txt=txt+'\nHASHED PNR NO: '+h[1]
                txt=txt+'\nPASSENGER\'S NAME: '+h[2]
                txt=txt+'\nAGE: '+h[4]
                txt=txt+'\nGENDER: '+h[5]
                txt=txt+'\nRESERVED ON: '+h[10]
                txt=txt+'\nNO OF SEATS: '+h[3]
                txt=txt+'\nTRAIN NO: '+h[6]
                txt=txt+'\nTRAIN NAME: '+conv1(h[6])
                txt=txt+'\nFROM STATION: '+conv(h[7])
                txt=txt+'\nTO STATION: '+conv(h[8])
                txt=txt+'\nCLASS: '+h[9]
                frmP.destroy()
                FRMP()
        ...
        def FRMG():
            ...
            # en1=Entry(frmI)
            # en1.pack()
            # bt1=Button(frmI,text='Find Details',command=lambda x=en1:check1(x))
            # bt1.pack()
            # def check1(a):
            #     global c
            #     a = a.get().strip()
            #     print(a)
            #     if a != '':
            #         pnr_file_path = r'PNRS\PNRS.txt'
            #         temp_file_path = r'PNRS\temp.txt'
            #         found = False

            #         with open(pnr_file_path, 'r') as fin, open(temp_file_path, 'w') as temp:
            #             for line in fin:
            #                 p = line.strip('\n')
            #                 if a == p:
            #                     # show(p)
            ...
            #         if found:
            #             os.remove(pnr_file_path)
            #             os.rename(temp_file_path, pnr_file_path)
            #         else:
            #             frmI.destroy()
            #             FRMI()
            #             lb = Label(frmI, text='\n\nINVALID PNR!!!')
                    ...
                    if a8==''or a8 not in ['1A','2A','3A','SL']:
                        ...
                        if x[0][ind]>=p[ind]:
                            ...
                            while True:
                                ...
                                hash_object.update(pnr1.encode('utf-8'))
                                hashedcode=hash_object.hexdigest()
                                ...
                                if str(pnr) not in f.read():
                                    ...
                                    f.writelines(str(pnr)+'\n')
                                    f.flush()
                                    break
                            fin=open(r'PASSENGER\PASSENGER.txt','a')
                            txt= str(pnr)+','+str(hashedcode)+','+a1+','+a2+','+a3+','+a4+','+a5+','+a6+','+a7+','
        ...
        #----------LABEL BUTTONS AND ENTRIES----------
            ...
            en1=Entry(frmI)
            en1.pack()
            bt1=Button(frmI,text='Cancel Ticket',command=lambda x=en1:check1(x))
            bt1.pack()
            def check1(a):
                global c
                a = a.get().strip()
                ...
                if a != '':
                    pnr_file_path = r'PNRS\PNRS.txt'
                    temp_file_path = r'PNRS\temp.txt'
                    pass_file_path = r'PASSENGER\PASSENGER.txt'
                    temp1_file_path = r'PASSENGER\temp1.txt'
                    found = False
                    found1=False
                    ...
                    with open(pnr_file_path, 'r') as fin, open(temp_file_path, 'w') as temp:
                        for line in fin:
                            ...
                            if a == p:
                                ...
                                temp.write(line)
                    with open(pass_file_path, 'r') as fin, open(temp1_file_path, 'w') as temp:
                        for line in fin:
                            p = line.strip('\n')
                            if a == p.split(',')[0]:
                                # show(p)
                    ...
                    if found1:
                        ...
                        os.rename(temp1_file_path, pass_file_path)
                    else:
                        frmI.destroy()
                        FRMI()
                        lb = Label(frmI, text='\n\nINVALID PNR!!!')
